...
class OSNet(nn.Module):
    ...
    def out_layers_extractor(self, x):
        if True:
            out_conv1 = self.conv1(x)
            out_maxpool = self.maxpool(out_conv1)
            out_conv2 = self.conv2(out_maxpool)
            return out_conv2
        else:
            out_conv1 = self.conv1(x)
            out_maxpool = self.maxpool(out_conv1)
            out_conv2 = self.conv2(out_maxpool)
            x = self.pool2(out_conv2)
            out_conv3 = self.conv3(x)
            x = self.pool3(out_conv3)
            x = self.conv4(x)
            out_featuremap = self.conv5(x)
            out_globalavg = self.global_avgpool(out_featuremap)
            v = out_globalavg.view(out_globalavg.size(0), -1)
            #out_fc = self.fc(v)
            return out_conv1, out_maxpool, out_conv2, out_conv3, out_featuremap, out_globalavg
